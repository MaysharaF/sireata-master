...
public class EditarAtaWindow extends EditarWindow {
	...
	public EditarAtaWindow(Ata ata, ListView parentView) {
		...
		HorizontalLayout h6 = new HorizontalLayout(this.btAdicionarPauta, this.btEditarPauta, this.btRemoverPauta,
				this.btPautaAcima, this.btPautaAbaixo);
		h6.setSpacing(true);
		...
		HorizontalLayout h5 = new HorizontalLayout(this.btAdicionarParticipante, this.btEditarParticipante,
				this.btRemoverParticipante);
		h5.setSpacing(true);
		...
		HorizontalLayout h7 = new HorizontalLayout(this.btVisualizarAnexo, this.btAdicionarAnexo, this.btEditarAnexo,
				this.btRemoverAnexo, this.btMoverAnexoAcima, this.btMoverAnexoAbaixo);
		h7.setSpacing(true);
	...
	private void carregarNumeroAta() {
		if (ata.getIdAta() == 0) {
			try {
				AtaBO bo = new AtaBO();
				int numero = bo.buscarProximoNumeroAta(this.cbOrgao.getOrgao().getIdOrgao(),
						DateUtils.getYear(this.dfData.getValue()), (TipoAta) this.cbTipo.getValue());
	...
	private void carregarParticipantes() {
		...
		for (AtaParticipante p : this.ata.getParticipantes()) {
			this.gridParticipantes.addRow(p.getParticipante().getNome(), p.getDesignacao(), (p.isMembro() ? "Sim" : "Não"),
					(p.isPresente() ? "Sim" : "Não"));
		}
	}
	...
	private void editarParticipante() {
		...
		if (this.indexParticipante == -1) {
			Notification.show("Editar Participante", "Selecione o participante para editar.",
					Notification.Type.WARNING_MESSAGE);
		} else {
			UI.getCurrent()
					.addWindow(new EditarParticipanteWindow(this.ata.getParticipantes().get(this.indexParticipante), this));
		}
	}
	...
	private void removerParticipante() {
		...
		if (index == -1) {
			Notification.show("Remover Participante", "Selecione o participante para remover.",
					Notification.Type.WARNING_MESSAGE);
		} else {
	...
	public void atualizarParticipante(AtaParticipante participante) {
		if (this.indexParticipante == -1) {
			...
			if (!encontrou) {
				...
				Notification.show("Adicionar Participante", "Usuário já consta na lista de participantes.",
						Notification.Type.WARNING_MESSAGE);
			}
		} else {
	...
	private void editarPauta() {
		...
		if (this.indexPauta == -1) {
			...
			UI.getCurrent().addWindow(new EditarPautaWindow(this.ata.getPauta().get(this.indexPauta),
					this.ata.isAceitarComentarios(), this.isBotaoSalvarVisivel(), this));
		}
	}
	...
	private void publicarAta() {
		try {
			ConfirmDialog.show(UI.getCurrent(), "Confirma a publicação da ata? Esta ação não poderá ser revertida.",
