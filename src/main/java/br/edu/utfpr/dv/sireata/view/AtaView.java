...
public class AtaView extends ListView {
	...
	private void visualizarPrevia() {
		...
		if (id == null) {
			Notification.show("Prévia da Ata", "Selecione uma ata para visualizar a prévia.",
	...
	protected void carregarGrid() {
		...
		try {
			AtaBO bo = new AtaBO();
			List<Ata> list = bo.listar(this.cbCampus.getCampus().getIdCampus(),
					this.cbDepartamento.getDepartamento().getIdDepartamento(), this.cbOrgao.getOrgao().getIdOrgao(),
					(this.tipo == 1), Session.getUsuario().getIdUsuario());

			for (Ata u : list) {
				Object itemId = this.getGrid().addRow(u.getOrgao().getNome(), u.getTipo().toString(),
						String.valueOf(u.getNumero()) + "/" + String.valueOf(DateUtils.getYear(u.getData())), u.getData(),
						u.getPresidente().getNome(), u.getSecretario().getNome());
		...
		try {
			...
			if (idAta == null) {
				...
				if (bo.isPresidente(Session.getUsuario().getIdUsuario(), (int) idAta)) {
					ConfirmDialog.show(UI.getCurrent(), "Confirma a exclusão da ata? Esta ação não poderá ser revertida.",
		...
		try {
			...
			if (idAta == null) {
				...
				if (bo.isPresidenteOuSecretario(Session.getUsuario().getIdUsuario(), (int) idAta)) {
								...
								public void onClose(ConfirmDialog dialog) {
									if (dialog.isConfirmed()) {
										try {
											...
											Notification.show("Publicar Ata", "Ata publicada com sucesso.",
													Notification.Type.WARNING_MESSAGE);
					...
					Notification.show("Publicar Ata", "Apenas o presidente ou secretário da reunião pode publicar a ata.",
							Notification.Type.WARNING_MESSAGE);
				}
			}
		} catch (Exception e) {
